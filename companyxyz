...
c	library(readxl)
	...
	library(tidyverse)
	library(car)
	companyxyzdata<-read_excel("C:/Users/Morgan's Laptop/Desktop/Trinity/Fall 20/Analytics/Student Data for Distribution.xlsx")
	...
	#EXPLORATION
	ggplot(companyxyzdata,aes(x=`Origin`))+geom_bar(aes(fill=`LOB`))
	'We found that product A and C are only shipped from site A'
	

	ggplot(companyxyzdata,aes(x=`Ship Date`))+geom_bar(aes(fill=`Ship Qtr`))
	'Exploring the dates of each ship quarter'
	ggplot(companyxyzdata,aes(x=`Ship Date`))+geom_density(aes(fill=`Ship Qtr`))
	ggplot(companyxyzdata,aes(x=`Receipt Date`))+geom_density(aes(fill=`Ship Qtr`))
	...
	companyxyzdata_nona<-na.omit(companyxyzdata)
	...
	goodDates <- companyxyzdata_nona %>%
	  filter(companyxyzdata_nona$`Ship Date` > companyxyzdata_nona$`PO Download Date` & companyxyzdata_nona$`Receipt Date` > companyxyzdata_nona$`PO Download Date` & companyxyzdata_nona$`Ship Date`< companyxyzdata_nona$`Receipt Date`) 
	

	badDates <- companyxyzdata_nona %>%
	  filter(!(companyxyzdata_nona$`Ship Date` > companyxyzdata_nona$`PO Download Date` & companyxyzdata_nona$`Receipt Date` > companyxyzdata_nona$`PO Download Date` & companyxyzdata_nona$`Ship Date`< companyxyzdata_nona$`Receipt Date`)) 
